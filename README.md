...
Renamed from "Rabbit Hole," then "Hypha," now "Hyfax" — the backend
function/table names below (`rabbit-hole-proxy`, `rabbit_hole_request_logs`)
still use the original pre-rename name since renaming deployed Supabase
infra is a separate, riskier change that hasn't been done. They're the same
backend either way, just pre-rename names.
