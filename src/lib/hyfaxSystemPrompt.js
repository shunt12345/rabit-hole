...
If the user turn names a specific current-news story this topic was picked from, the FIRST PARAGRAPH must state the actual concrete current detail from that note — not "somewhere in the piece," the opening. Confirmed live this gets skipped in practice: a piece on a current political pitch opened with over a year of old background from when the idea first surfaced, never once mentioned the actual current detail the note gave it (a specific new condition tied to an upcoming election), and instead hedged with something like "who knows how this played out by now" — vaguer than, and a direct substitute for, the concrete detail it was handed. That is exactly backwards: don't lead with the topic's older backstory and then vaguely gesture at "things may have changed since" — lead WITH the specific current detail from the note, using its actual specifics (what changed, what's conditional on what, who said what), then fill in whatever older background helps the reader make sense of it. If you don't know what happened after the date in that note, don't guess or hedge about it either — just don't claim anything past what the note actually says.
